<template>
  ...
  >
    <div class="space-y-3">
      ...
      <p class="llm-note text-xs">
        提示：这里填写 provider 的 modelId（如 `xopkimik25`），不是展示名称。
      </p>
      <p class="llm-note text-xs">
        识别提供商：
        <span
          class="llm-provider-chip"
          :class="providerToneClass"
        >{{ detectedProviderLabel }}</span>
      </p>
      ...
      <p
        v-if="message"
        class="llm-message text-sm"
      >
        {{ message }}
      </p>
      <p
        v-if="error"
        class="llm-error text-sm"
      >
        {{ error }}
      </p>
  ...
  <div
    v-else-if="isOpen"
    class="llm-overlay fixed inset-0 z-50 flex items-center justify-center p-4"
  >
    <section class="llm-modal w-full max-w-4xl rounded-2xl p-5">
      <header class="mb-3 flex items-center justify-between">
        <h3 class="llm-title text-xl font-display">
          LLM 模型配置
        </h3>
        <button
          class="llm-btn"
      ...
      <div class="llm-dialog-grid">
        <aside class="llm-left">
          <p class="llm-sub-title">
            设置分组
          </p>
          <div class="mt-2 grid gap-2">
            <button
              type="button"
              class="llm-chip llm-chip-active text-left"
            >
              模型配置
            </button>
          </div>
        ...
        <section class="llm-right">
          <div class="llm-inline-wrap p-3">
            <div class="space-y-3">
              <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <label class="llm-label text-sm">Endpoint<input
                  v-model="form.endpoint"
                  class="llm-input"
                ></label>
                <label class="llm-label text-sm">模型ID（model）<input
                  v-model="form.model"
                  class="llm-input"
                ></label>
              </div>
              ...
              <label class="llm-label text-sm">API Key<input
                v-model="form.apiKey"
                type="password"
                class="llm-input"
              ></label>
              <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <label class="llm-label text-sm">maxTokens<input
                  ...
                  class="llm-input"
                ></label>
                <label class="llm-label text-sm">temperature<input
                  v-model.number="form.temperature"
                  type="number"
                  min="0"
                  max="2"
