...
function controlledOutputRequestId(proposalId: string, scope: NoNameApplyScope): string {
  return `controlled-output-${proposalId}-${webNoNameScopeTraceKey(scope)}`;
}

function controlledOutputKindForScope(scope: NoNameApplyScope): NoNameControlledOutputKind {
  return webNoNameScopeKind(scope);
}
