...
| 3. 数据迁移与兼容策略 | 未完成 | 部分完成 | 已新增 `docs/release/migration-v2.md`；待实现显式版本迁移器与日志落盘 |
...
| 任务 | 计划状态 | 代码审计状态 | 证据 |
|---|---|---|---|
| 23. 属性测试与回归扩展 | 未完成 | 部分完成 | 已有 property tests（`src-tauri/src/plot_engine.rs`），但未覆盖全部新增不变量 |
| 24. 性能目标 | 未完成 | 未完成 | 无基准报告与 P95 数据文档 |
| 25. 发布准备 | 未完成 | 部分完成 | 有测试报告与打包产物，缺少完整迁移说明/已知限制发布文案 |
