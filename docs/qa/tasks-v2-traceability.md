...
| 任务 | 计划状态 | 代码审计状态 | 证据 |
|---|---|---|---|
| 10. 功法语义模型 | 未完成 | 部分完成（增强中） | 已将灵根适配/高阶门槛/高风险功法语义接入战斗结果修正与后果，并保留 `TechniqueDef` 结构化字段；角色面板已新增“功法流派”分组展示。仍待更完整学习链路（`src-tauri/src/tauri_commands.rs`, `src/components/CharacterPanel.vue`） |
| 11. 学习/突破/走火入魔链路 | 未完成 | 部分完成（增强中） | 已接入突破失败后果链路：气机紊乱累积、风险功法反噬增伤、走火入魔临界预警；并新增“气机紊乱≥8 时禁止强行突破”的门槛规则，写入成长日志与事件（`src-tauri/src/game_state.rs`, `src-tauri/src/tauri_commands.rs`）；仍待完整学习条件/偏差分支体系 |
...
| 13. 移动与遭遇机制 | 未完成 | 部分完成（增强中） | 已新增移动命令 `travel_to_location` + `get_reachable_locations`，接入可达性约束、时间消耗（+1日）、按灵气/仇恨/气机紊乱加权的基础遭遇触发与事件日志回写；并引入 `travel_rules_v2.json` 配置化参数与分级遭遇文本池。前端地图页签可触发移动并显示可达性。仍待完整路径图（`src-tauri/src/tauri_commands.rs`, `src-tauri/src/travel_rules.rs`, `src-tauri/config/travel_rules_v2.json`, `src/components/InfoTabsDialog.vue`, `src/components/GameView.vue`） |

## 阶段 F

| 任务 | 计划状态 | 代码审计状态 | 证据 |
|---|---|---|---|
| 14. 战斗判定引擎 V2 | 未完成 | 部分完成（增强中） | 已接入硬约束校验（数值守门）、功法语义克制、行为策略匹配（谨慎/强攻/保命）、环境修正（基于当前位置灵气与伤势/气机）以及流派克制矩阵（剑/刀/体/符阵标签）；克制分值已配置化（`src-tauri/src/numeric_guard.rs`, `src-tauri/src/tauri_commands.rs`, `src-tauri/src/combat_style_rules.rs`, `src-tauri/config/combat_style_rules_v2.json`）。仍待更完整兵器实体矩阵 |
...
| 任务 | 计划状态 | 代码审计状态 | 证据 |
|---|---|---|---|
| 20. 成长面板与战斗复盘 | 未完成 | 部分完成（增强中） | 角色面板已展示战后状态与最近成长记录；信息面板新增“战斗复盘”页签，可回看近期 `combat_explanation/combat/encounter` 事件；仍待更完整图形化复盘视图（`src/components/CharacterPanel.vue`, `src/components/InfoTabsDialog.vue`, `src/components/GameView.vue`, `src-tauri/src/tauri_commands.rs`） |
| 21. 地图与行程 UI | 未完成 | 部分完成（增强中） | 信息面板地图页签已接入结构化地图总览（可达性、风险层级、灵气差）并支持移动交互；仍待可视化路径图与热区层（`src-tauri/src/tauri_commands.rs`, `src/components/InfoTabsDialog.vue`, `src/components/GameView.vue`, `src/stores/gameStore.ts`） |
