...
| 任务 | 计划状态 | 代码审计状态 | 证据 |
|---|---|---|---|
| 14. 战斗判定引擎 V2 | 未完成 | 部分完成（增强中） | 已接入硬约束校验（数值守门）、功法语义克制、行为策略匹配（谨慎/强攻/保命）与环境修正（基于当前位置灵气与伤势/气机），并写入战斗解释；仍待兵器克制矩阵（`src-tauri/src/numeric_guard.rs`, `src-tauri/src/tauri_commands.rs`） |
