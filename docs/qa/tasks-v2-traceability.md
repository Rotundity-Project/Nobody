...
| 15. 战斗结果解释器 | 未完成 | 部分完成 | 有 reason 文本拼接，未见独立解释器与主导/反转因素结构化输出 |
| 16. 战斗与剧情耦合 | 未完成 | 部分完成 | 事件日志与剧情推进有耦合，伤势/威望/仇恨等长期状态影响不完整 |
...
| 任务 | 计划状态 | 代码审计状态 | 证据 |
|---|---|---|---|
| 23. 属性测试与回归扩展 | 未完成 | 部分完成（增强中） | 已新增章节目录完整性、导出事实覆盖率、地图/数值边界、功法适配不变量测试（`src-tauri/src/novel_generator.rs`, `src-tauri/src/numeric_guard.rs`, `src-tauri/src/entity_validator.rs`） |
