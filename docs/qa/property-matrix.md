...
| Technique risk-tag traceability in chapter export | `prop_export_keeps_technique_risk_tags_traceable` | `src-tauri/src/novel_generator.rs` | `cargo test -q prop_export_keeps_technique_risk_tags_traceable` |
...
## Remaining Gaps
- No blocker-level gaps for Task 23 core invariants; remaining work is to持续扩大真实回归样本库。
